...
public class PaymemtCartServiceImpl implements PaymentCartService {
	...
	public String doOrderCart(DoOrderDto doOrderDto, HttpSession session) {
		...
		if (result == 0) {
			...
			// 적립포인트
			int increpoint = Math.floorDiv(total, 100); // 회원이 구매확정누르면 되도록...
			...
			try {
				for (int i = 0; i < plist.size(); i++) {
					String pro_num = plist.get(i).getPdto().getPro_num();// 구매상품
					int count = plist.get(i).getBmdto().getCount();// 구매개수
					pdao.updateProductAmount(pro_num, count);// 구매한 만큼 재고반영
				}
			} catch (Exception e) {
				check = -10;// 재고 반영
				System.out.println("재고반영실패");
			}
