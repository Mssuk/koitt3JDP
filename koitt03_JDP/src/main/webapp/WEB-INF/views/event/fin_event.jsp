		...
		<div id="outbox">		
			...
			<div id="contents">
				<div id="mypage">
					...
					<c:if test="${text!='' }">
						<h3><span>'<em class="keyword">${text }</em>'에 대한 종료된이벤트 내 검색 결과입니다.</span>						</h3>
					</c:if>
					<!-- list -->
					<div class="eventList">
							<ul>
								...
								<c:otherwise>
									...
									<c:forEach var="dtos" items="${list }">
										<li>
											<c:choose>
												<c:when test="${text!='' }">
													<a href="fin_event_view?event_num=${dtos.event_num }&pageNum=${pageNum}&search=${search}&text=${text}" title="${dtos.event_title }">	
													<div class="img">
												...
												<c:otherwise>
													<a href="fin_event_view?event_num=${dtos.event_num }&pageNum=${pageNum}" title="${dtos.event_title }">
														<div class="img">
															<img src="${dtos.event_image1 }" alt="종료된 이벤트" />
														</div>
														<div class="txt">
